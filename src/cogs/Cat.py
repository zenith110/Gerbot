...
class Cat(commands.Cog):
    ...
    async def GetCat(
        ...
        if arg is None:
            ...
            await context.send(
                f"{context.author.mention}, here is your {cat_name} that you asked for!"
            )
            cat_message = discord.Embed(
                title=cat_name[0].upper() + cat_name[1:],
                description="Beep beep, I am a " + cat_name,
            )
            ...
            try:
                ...
                await context.send(
                    "The API we use doesn't have that breed, check out https://api.thecatapi.com/v1/breeds for a full list"
                )
    ...
    async def AllCats(self, context: discord.ext.commands.context.Context):
        ...
        cat_message = discord.Embed(
            title="Cat breeds", description="Beep beep, here are all the cat's ids"
        )

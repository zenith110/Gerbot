...
class MLH(commands.Cog):
    ...
    async def getMlh(self, context):
        ...
        mlh_message = discord.Embed(
            title="MLH events", description="Current MLH events for the 2020 year"
        )
        for i in range(0, 17):
            mlh_message.add_field(
                name=str(mlh_obj.dates[0][i]),
                value="["
                + str(mlh_obj.names[0][i])
                + "]("
                + str(mlh_obj.links[0][i])
                + ")\n"
                + str(mlh_obj.cities[0][i])
                + ", "
                + str(mlh_obj.states[0][i]),
                inline=True,
            )
    ...
    async def getCurrentMlh(self, context):
        ...
        mlh_message = discord.Embed(
            title=full.strftime("%b") + " " + str(year) + " hackathons",
            description="Current MLH events coming up this month",
        )
        for i in range(0, len(mlh_obj.current_month_events)):
            mlh_message.add_field(
                name=full.strftime("%b"),
                value=mlh_obj.current_month_events[i],
                inline=True,
            )
        await context.send(embed=mlh_message)
